...
{   
    ...
    {   
        ...
        //得到首个地图点
        //获取该地图点的顺序和坐标
        if(path_length!= 0 && i < ((path_length-1)*5+1))
        {
            //处理路径
            if(path_flag == 0)
            {
                for(int k = 0; k < (path_length-1); k++)
                {   
                    double dx = (Ori_path.poses[k+1].pose.position.x - Ori_path.poses[k].pose.position.x)/5;
                    double dy = (Ori_path.poses[k+1].pose.position.y - Ori_path.poses[k].pose.position.y)/5;
                    for(int l =0; l < 5; l++)
                    {
                        geometry_msgs::PoseStamped pose_2;
                        pose_2.header.frame_id = "map"; // 设置参考坐标系
                        pose_2.header.stamp = ros::Time::now();
                        pose_2.pose.position.x = Ori_path.poses[k].pose.position.x + dx*l;
                        pose_2.pose.position.y = Ori_path.poses[k].pose.position.y + dy*l;
                        pose_2.pose.position.z = 0;
                        pose_2.pose.orientation.w = 1.0;
                        Path_nodes.poses.push_back(pose_2);
                    }
                }
                ...
                pose_2.header.frame_id = "map"; // 设置参考坐标系
                pose_2.header.stamp = ros::Time::now();
                pose_2.pose.position.x = Ori_path.poses[path_length-1].pose.position.x;
                pose_2.pose.position.y = Ori_path.poses[path_length-1].pose.position.y;
                pose_2.pose.position.z = 0;
                pose_2.pose.orientation.w = 1.0;
                Path_nodes.poses.push_back(pose_2);
                

                Path_nodes.header.stamp = ros::Time::now(); // 更新路径的时间戳
                Path_nodes.header.frame_id = "map";
                path_flag = 1;
            }
            path_length_o = Path_nodes.poses.size();
            ...
            pre_avel = robot_motion_LQR.lqrControl(state, ref_path, A_B[0], A_B[1], Q_R[0], Q_R[1]);
